...
## Campaign
- **Exit:** `FinalPit` is a trigger baked into the scene. Entering it sets `NewMovement.levelOver`, clears `activated`, calls `StatsManager.StopTimer()` and sends results 5 s later. Decoys carry `fakeEnd`, `secondPit` or `rankless`. The pit can sit in a room that starts inactive, so the mod searches with `FindObjectsOfType<FinalPit>(true)`. `targetLevelName` is the scene it leads to; a secret level's name ends in `-S`. A level can hold several real-looking pits (0-2: one to `Level 0-3` and one to `Level 0-S`; 1-1: one to `Level 1-2` and two to `Level 1-S`), all inactive at load, so the mod filters the secret targets out and then prefers the next mission in campaign order.
