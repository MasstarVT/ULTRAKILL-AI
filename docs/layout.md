...
- `python/tests/test_progress.py`: `ProgressCallback`, dashboard and `poll_status.py` tests against fake Cyber Grind and campaign envs (no game needed). Includes the difficulty clauses: a change empties the rolling windows, the mod's `-1` sentinel does not, and a `metrics_log.csv` whose header is missing a column is rotated aside rather than appended to.
- `python/tests/test_poll_status.py`: `poll_status.py`'s header handling on its own — when a `metrics_log.csv` is rotated aside (a column it writes has no place in the header) and when it is left alone (every column fits, or the header merely has extra ones), that the old log is kept, and that `campaign.difficulty` survives the rotation into the column `keep_best.py --metric time` ranks on (no game needed; `python tests/test_poll_status.py`).
...
- `python/scripts/sync_agents_md.py` — regenerates the root `AGENTS.md` from `CLAUDE.md` (`--check` exits 1 on drift); `python/tests/test_agents_md.py` fails the suite when the two differ, which is what keeps `AGENTS.md` current.
